...
In this paper, we measured key variables related to voter behaviour. From the dataset, we selected `gender`, `race`, `gunown`. In Figure 1, we can see support for candidate based on gender and race. For gender, we considered two options, male or female. For race, we considered 8 options, White, Black, Hispanic, Asian, Native American, Middle Eastern, two or more races, and other. Note that only one selection was allowed in the original study for both gender and race. Figure 2 illustrates the support for each candidate based on gun ownership status, with four options. Individuals who personally own a gun, who don’t personally own one, but someone in their household owns a gun, no one in the household owns a gun, and not sure. We also selected the variable that recorded whether an individual voted for Biden or Trump in the 2020 Presidential election, `CC20_410`. This allowed us to establish a binary outcome variable for the probability of voting Biden or Trump in our model in @sec-model. The variables measured here allowed us to identify variation in political attitudes and voting patterns. 

Data was downloaded and cleaned to suit our needs and during the data cleaning process, we made sure to only include participants who were adults registered to vote. We also removed rows with NA values for `gunown`. This was due to the fact that we wanted to see individuals who answered all the questions for our variables of interest. We almost limited the participants to those who explicitly stated they voted for Biden or Trump. Other options were available to participants such as “I did not vote” and “not sure”.
